...
Before we implement this function for all the types, we should specify a few
helper functions which will make our life easier, especially for operators with
equivalent stack behavior.
...
> evaluateIntBin ::  (Integer -> Integer -> Either String Integer) -> Display -> (Stack V.Value)
>                    -> Either String (Display, Stack V.Value)
> evaluateIntBin f d stk
>   | (length operands) < 2     = Left $ nope "stack too shallow"
>   | not (V.isInteger lopval)  = Left $ nope "left operand is not an integer"
>   | not (V.isInteger ropval)  = Left $ nope "right operand is not an integer"
>   | isLeft reseither          = Left $ fromLeft reseither
>   | otherwise                 = Right (d, push (V.CInteger $ fromRight reseither) stn)
>   where
>     nope s            = "cannot apply binary operation: " ++ s
...
Some aren't. We provide versions that handle this depressing fact gracefully.
...
The next one does the same for \textit{and} and \textit{or}.
...
> intStackOp ::  (Integer -> Stack V.Value -> Either String (Stack V.Value))
>                -> Display -> (Stack V.Value)
>                -> Either String (Display, (Stack V.Value))
> intStackOp op d stk
...
> intParOp ::  (Integer -> V.String8 -> Either String V.Value) -> Display -> (Stack V.Value)
>              -> Either String (Display, (Stack V.Value))
> intParOp op d stk
>   | (length operands) < 2  = Left $ nope "stack too shallow"
>   | lirp                   = packRes $ op loi rop
>   | rilp                   = packRes $ op roi lop
>   | otherwise              = Left $ nope "one operand must be an integer, "
>                                ++ "the other a parenthesized expression"
>   where
>     nope s             = "cannot apply parenthesized expression operation: " ++ s
>     (operands, stn)    = multipop 2 stk
>     loval              = operands !! 0
>     roval              = operands !! 1
...
The functions relevant to the above are \textit{char} and \textit{append}:
...
\textit{copy} and \textit{delete} perform the respective stack operations.

> evaluateOne V.CCopy    d s = intStackOp (pickE)     d s
> evaluateOne V.CDelete  d s = intStackOp (middropE)  d s

\textit{evaluate} pops a parenthetized expression, parses it, and pushes the
resulting stack onto the current stack. If the top of the stack is not a
parenthetized expression, leave it unchanged.
...
\textit{char} and \textit{append} perform the respective
integer-and-parenthesized-expression operations.

> evaluateOne V.CChar    d s = intParOp (parChar)    d s
> evaluateOne V.CAppend  d s = intParOp (parAppend)  d s

\textit{output} is the only one that changes the display.
...
The full evaluation function simply performs step-by-step evaluation until the
stack stops changing.
