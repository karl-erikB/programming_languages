...
An opening parenthesis starts a string. Push an empty one for the string parser
to extend.
...
If the controlled parser hits a newline, it pushes the string as a parenthesized
expression and continues parsing with an empty string.
...
>   | (i < 0) || (i > 255)  = Left errStr
>   | otherwise             = controlledParse (x:xs) cs stk
>   where
>     i       = fromEnum c
>     x       = toEnum i
>     errStr  = "invalid character found (codepoint " ++ show i ++ ")"
