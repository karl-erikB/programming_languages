...
depth simply returns the length of the underlying list.

> depth :: (Integral n) => Stack a -> n
> depth (Stack xs) = genericLength xs

push is simple---just add the element to the front.
...
Let's also implement functions to peek at or pop multiple elements. Those will
simply return a shorter list if the stack is too shallow.
...
Finally, a few middle-of-the-stack operations. middrop deletes a value in the
middle of the stack.
...
pick chooses a value from the middle of the stack and copies it to the top.
